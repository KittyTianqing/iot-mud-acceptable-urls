...
device makes use of Internet resources, network access permission and associated suggested network behavior, which are described in a MUD file hosted in its manufacturer's MUD file server. By providing a MUD URL, the network manager can locate this MUD file.
...
the network behavior of the device may change with time or upgrade. Therefore, the rules defined in the MUD file need to be changed correspondingly. Not all devices upgrade at the same time. Therefore, directly modifying the MUD file may affect the devices that are not upgraded.

This draft discusses the issues of current methods when facing the upgrading scenario and propose a simpler solution.
...
The published description (i.e. MUD file) is updated whenever the behavior of the firmware changes.

Three scenarios for updating the MUD file and and corresponding possible issues are discussed below.
...
The new MUD file would provide for such access, but when combined with the weak security of the old firmware, resulted in a compromised device.
...
The owner of a vehicle may desire to only perform software upgrades when they are at home, and could make other arrangements for transposition, rather than when they are parked at a remote cabin.
...
In this case the old device will be performing unwanted connections, and the MUD controller will be alerting the device owner that the device is mis-behaving rather than not upgraded.
...
However, it is difficult for manufacturers to update the IDevID of a device which is already in a box. Although the IDevID mechanism makes the MUD URL in the IDevID certificate unchangeable, it can be assumed that a new MUD URL is carried after the device is updated. If the new MUD URL matches the MUD URL in IDevID, the new MUD URL can be considered reasonable.

The DHCP and LLDP mechanisms are not signed, but are asserted by the device. They can be easily updated when the firmware is updated. Because of that sensitivity, they may also be easily changed by  sufficiently well targeted malware.
...
There are mitigating mechanisms which may be enough to deal with this trust issue when MUD files are signed by the manufacturer.
...
Besides  IDevID mechanism and other proper method, Trust on First Use (TOFU) mechanism,  which is popular for many MUD controllers currently, can be applied to trust the previous MUD file signer.
The first time a signature from a particular device-type is verified, the identity of the signing authority is recorded.
It is pinned.
Subsequent MUD file must be signed by the same entity in order to be accepted.
...
In this case, the question is that whether the MUD controller should pin the End-Entity(EE) certificate, or the CA certificate.
...
via Trust on First Use with DHCP or LLDP or another mechanisms.
...
the MUD file itself located on the vendor's web site.
...
It may be recommended that MUD URL uses a random character string instead of the plain text information.
...
Another security risk is that there is no clear way to connect MUD file signer to the device.
